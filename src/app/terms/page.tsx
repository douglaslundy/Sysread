  ...
  return (
    <PublicPage platformName={platformName} title="Terms of Service" eyebrow={"Effective " + legalIdentity.effectiveDate}>
      <div className="legal-copy">{legalIdentity.termsText ? legalIdentity.termsText.split(/\n\s*\n/u).map((paragraph, index) => <p key={index}>{paragraph}</p>) : <>
        ...
        <p>These terms govern access to {platformName}, operated by {legalIdentity.operatorName}. By creating an account or using the service, you agree to these terms and confirm that you can legally enter this agreement.</p>

        <h2>Account and acceptable use</h2>
        <p>You are responsible for your account activity and for keeping your credentials secure. Do not misuse the service, evade limits, upload malware, interfere with other users, attempt unauthorized access, or use {platformName} for unlawful activity.</p>

        <h2>Your content</h2>
        <p>You retain ownership of material you import. You grant {platformName} the limited permission required to store, parse, clean, simplify and display that material for you. Import only content that you own or are authorized to use. You are responsible for respecting copyright and other third-party rights.</p>
        ...
        <p>Features may change or be suspended for maintenance, security, provider availability or legal compliance. Accounts that materially violate these terms may be restricted or terminated. You may stop using {platformName} and request account deletion at any time.</p>
